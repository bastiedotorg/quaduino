//
// Created by bastian on 07.07.21.
//

#ifndef QUADIUNO_RS485_H
#define QUADIUNO_RS485_H

#include "datatypes.h"

#define RS485_ACTIVATION_PIN_IN_3    5   //RS485 Modul an RX/TX
#define RS485_ACTIVATION_PIN_OUT   2   //RS485 Modul an RX1/TX1
#define RS485_ACTIVATION_PIN_IN_1   3   //RS485 Modul an RX2/TX2
#define RS485_ACTIVATION_PIN_IN_2   4   //Rs485 Modul an RX3/TX3

/**
 *message length in byte
 */
#define MESSAGE_LENGTH 10

#define RS485_BAUD_RATE 9600

/**
 * in ms
 */
#define RS485_TIMEOUT 10

typedef enum {
    TYPE_START = 0,
    XSYNCHRO,
    XCONMA,
    SUPMA,
    TYPE_END = 255
} MESSAGE_TYPE;
#define MESSAGE_TYPE_COUNT 3

typedef enum {
    STATUS_START = 0,
    MESSAGE_EMPTY,
    MESSAGE_NEW,
    MESSAGE_CONSUMED,
    MESSAGE_CORRUPTED,
    MESSAGE_OLD,
    STATUS_END = 255,
} MESSAGE_STATUS;

typedef struct {
    union {
        UINT64 asUint;
        SINT64 asInt;
        SINT8 asString[8];
        FLOAT64 asFloat;
        SINT32 asIntArray[2];
    } payload;
    MESSAGE_TYPE messageType; // should be uint8
    MESSAGE_STATUS messageStatus;
    UINT8 crc;
} MESSAGE;

UINT8 crc8(UINT8 const *, UINT8);

MESSAGE prepareMessage(unsigned int payload);

void rs485Init();
SINT8 sendMessageUint64(UINT64, MESSAGE_TYPE);

SINT8 receiveMessage(UINT8, MESSAGE *);

MESSAGE getSamplingMessage(UINT8, MESSAGE_TYPE);

MESSAGE receiveQueuingMessage(UINT8, MESSAGE_TYPE);

void cyclicCheckMessages();

static UINT8 const crc8_table[] = {
        };

#define TRANSMIT_OK 1
#define TRANSMIT_ERR -1

#endif //QUADIUNO_RS485_H
